...
export const FREE_CONVERSATION_SYSTEM_INSTRUCTION = `
...
export const TRANSCRIBE_EXACT_PROMPT =
  'Transcribe this audio exactly as spoken. Only output the transcription, nothing else.';

export const TRANSCRIBE_AND_CLEANUP_PROMPT = `Listen to this audio and produce two versions of the transcript:
...
export const TEF_AD_IMAGE_PROMPT = `Look at this advertisement image. Please respond with a JSON object containing:
...
export const TEF_QUESTIONING_IMAGE_PROMPT = `Look at this advertisement image. Please respond with a JSON object containing:
1. "summary": A concise 2-3 sentence description of what the advertisement is for, what product or service it promotes, and its key selling points or tagline if visible.
...
export function ttsSystemPrompt(text: string): string {
  const sanitizedText = text.replace(/<\/text>/g, '<\\/text>');
  return `You are to read out the following text in a friendly, encouraging tone. When speaking French, use a natural French accent. You MUST output ONLY AUDIO, not TEXT. Again, ONLY AUDIO, not TEXT. Here's the text enclosed in <text> tags: <text>${sanitizedText}</text>`;
}
