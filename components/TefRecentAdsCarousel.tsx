  ...
  useEffect(() => {
    ...
    const load = () => {
      ...
      void listSavedAds(exerciseType)
        .then((loaded) => {
          if (!cancelled && token === requestToken) setAds(loaded);
        })
        .catch(() => {
          if (!cancelled && token === requestToken) setAds([]);
        });
    };
    load();
  ...
  return (
    <section>
      ...
      <div className="flex items-center gap-2 mb-3">
        ...
        <h3 className="text-sm font-medium text-parle-navy-900">Recent</h3>
        <span className="text-[10px] text-parle-navy-300">({ads.length})</span>
      </div>
      ...
      <div className="flex gap-3 overflow-x-auto snap-x snap-mandatory pb-2 -mx-1 px-1 scroll-pl-1">
        {ads.map((ad) => (
          <div
            key={ad.id}
            className={`flex-shrink-0 snap-start w-[calc((100%-0.75rem)/1.5)] min-w-[9.5rem] max-w-[12rem] rounded-xl border bg-white overflow-hidden ${
              ad.id === mostRecentId ? 'border-parle-blue-500/60' : 'border-parle-navy-100'
            }`}
          >
            <img
              src={ad.imageDataUrl}
            ...
            <div className="p-2.5">
              <div className="flex items-center justify-end mb-1.5">
                <button
                  ...
                  className="text-parle-navy-300 hover:text-parle-navy-700 p-0.5"
                >
              ...
              <div className="text-[10px] text-parle-navy-300 mb-2">
              ...
              <div className="flex gap-1.5">
                <button
                  type="button"
                  onClick={() => onStart(ad)}
                  className="flex-1 h-7 rounded bg-parle-blue-500 hover:bg-parle-blue-600 text-white text-[10px] font-medium flex items-center justify-center gap-1"
                >
                  <svg xmlns="http://www.w3.org/2000/svg" className="h-3 w-3" viewBox="0 0 20 20" fill="currentColor">
                ...
                <button
                  type="button"
                  onClick={() => onTopics(ad)}
                  className="px-2 h-7 rounded border border-parle-navy-200 text-parle-navy-700 text-[10px] hover:bg-parle-blue-50"
                >
                  Topics
                </button>
              </div>
