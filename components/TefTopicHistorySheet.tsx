    ...
    <div className="fixed inset-0 bg-slate-900/95 z-[60] flex items-center justify-center p-4 overscroll-none">
      ...
      >
        <div className="flex justify-between items-start p-5 border-b border-slate-700 flex-shrink-0">
          <div>
            <h2 id="tef-topic-history-title" className="text-lg font-semibold text-slate-100">
            ...
            <>
              {archives.length === 0 ? (
                <div className="p-4 rounded-xl border border-dashed border-slate-700 text-center">
                  <svg xmlns="http://www.w3.org/2000/svg" className="h-4 w-4 mx-auto text-slate-500 mb-1" viewBox="0 0 20 20" fill="currentColor">
                        ...
                        {items.map((archive) => {
                          ...
                          return (
                            <button
                              ...
                              className="w-full flex items-center gap-3 p-3 rounded-xl bg-slate-700/50 border border-slate-600 hover:border-slate-500 text-left transition-colors"
                            >
                              {ad?.imageDataUrl ? (
                                <img
                                  src={ad.imageDataUrl}
                                  alt=""
                                  className="h-12 w-12 rounded-lg object-cover flex-shrink-0 border border-slate-600"
                                />
                              ) : (
                                <div className="h-12 w-12 rounded-lg bg-slate-700 flex-shrink-0 border border-slate-600" />
                              )}
                              ...
                              <svg xmlns="http://www.w3.org/2000/svg" className="h-4 w-4 text-slate-500 flex-shrink-0" viewBox="0 0 20 20" fill="currentColor">
              ...
              <div className="flex items-center gap-3">
                {selectedAd?.imageDataUrl ? (
                  <button
                    ...
                    className="relative group h-14 w-14 flex-shrink-0 rounded-lg overflow-hidden border border-slate-600 hover:border-slate-400 transition-colors focus:outline-none focus:ring-2 focus:ring-blue-500"
                  ...
                  <div className="h-14 w-14 rounded-lg bg-slate-700 border border-slate-600" />
                )}
                <div>
                ...
                <div className="flex items-center gap-2">
                  {onRestartSavedAd && selectedAd && (
                    <button
                      ...
                      className="text-xs font-medium text-white px-3 py-1.5 rounded-lg bg-green-600 hover:bg-green-500"
                    >
                      Restart
                    </button>
                  )}
                  <button
                    type="button"
                    onClick={() => setSelectedId(null)}
                    className="text-xs text-slate-300 hover:text-slate-100 px-3 py-1.5 rounded-lg border border-slate-600"
                  >
                    Back
                  </button>
                </div>
        ...
        {!selected && archives.length > 0 && (
          <div className="p-5 border-t border-slate-700 flex-shrink-0">
            <button
              type="button"
              onClick={onClose}
              className="w-full py-2 text-sm text-slate-300 hover:text-slate-100 rounded-lg border border-slate-600"
            >
              Close
            </button>
          </div>
