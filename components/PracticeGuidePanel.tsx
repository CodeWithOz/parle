...
export const PracticeGuidePanel: React.FC<PracticeGuidePanelProps> = ({ topics }) => {
  ...
  return (
    <div className="rounded-xl border border-violet-500/40 bg-violet-500/5">
      <button
        type="button"
      ...
      >
        <span className="text-violet-400 text-sm" aria-hidden="true">
          <svg xmlns="http://www.w3.org/2000/svg" className="h-4 w-4" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth={2}>
            <path strokeLinecap="round" strokeLinejoin="round" d="M12 6.253v13m0-13C10.832 5.477 9.246 5 7.5 5S4.168 5.477 3 6.253v13C4.168 18.477 5.754 18 7.5 18s3.332.477 4.5 1.253m0-13C13.168 5.477 14.754 5 16.5 5c1.747 0 3.332.477 4.5 1.253v13C19.832 18.477 18.247 18 16.5 18c-1.746 0-3.332.477-4.5 1.253" />
          </svg>
        </span>
        <div className="flex-1 min-w-0 text-left">
          <div className="text-sm font-semibold text-violet-300">Practice guide</div>
          <div className="text-[11px] text-slate-400">
            {topics.length} topics · from your last review
          </div>
        </div>
        <svg
          className={`w-4 h-4 text-violet-400 transition-transform duration-200 ${panelOpen ? 'rotate-180' : ''}`}
          fill="none"
          viewBox="0 0 24 24"
          stroke="currentColor"
          aria-hidden="true"
          ...
          {topics.map((t) => {
            ...
            return (
              ...
              >
                ...
                >
                  <span className="text-xs font-medium text-violet-300">{t.topic}</span>
                  <svg
                ...
                {isOpen && (
                  <div className="px-2.5 pb-2.5 space-y-1.5">
                    {t.examples.map((ex, idx) => (
                      <div key={idx}>
                        <div className="text-xs text-slate-100">{ex.french}</div>
                        <div className="text-[11px] text-slate-400">{ex.english}</div>
                      </div>
                    ))}
                  </div>
                )}
